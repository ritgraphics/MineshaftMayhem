...
D3D12Context::D3D12Context(HWND window) : m_backBufferFormat(DXGI_FORMAT_B8G8R8A8_UNORM), m_window(window)

{
    ...
    for (UINT i = 0; i < FRAME_COUNT; ++i)
    {
        ...
        std::wstring name = std::format(L"D3D12Context::CommandAllocator{}", i);
        ...
        name = std::format(L"D3D12Context::CommandList{}", i);
        ...
        name = std::format(L"D3D12Context::FrameFence{}", i);
    ...
    m_factory->MakeWindowAssociation(window, DXGI_MWA_NO_WINDOW_CHANGES);
}
...
void D3D12Context::PrepareWork(ID3D12CommandAllocator* commandAllocator, ID3D12GraphicsCommandList* commandList)
{
    ...
    const auto rtvHandle = GetRenderTargetView();
    const auto dsvHandle = GetDepthStencilView();
...
CD3DX12_CPU_DESCRIPTOR_HANDLE D3D12Context::GetRenderTargetView() const
{
    return CD3DX12_CPU_DESCRIPTOR_HANDLE(m_rtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(),
                                         m_currentBackBufferIndex, m_rtvDescriptorSize);
}

CD3DX12_CPU_DESCRIPTOR_HANDLE D3D12Context::GetDepthStencilView() const
{
...
void D3D12Context::CreateSurfaceResources()
{
    ...
    for (UINT i = 0; i < FRAME_COUNT; i++)
    {
        ...
        std::wstring name = std::format(L"D3D12Context::RenderTarget{}", i);
...
ID3D12CommandAllocator* D3D12Context::GetCommandAllocator() const
{
    return m_commandAllocator[m_currentBackBufferIndex].get();
}
...
const D3D12Context::StaticSamplers D3D12Context::GetSamplers() const
{
